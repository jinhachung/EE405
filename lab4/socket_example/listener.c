...
int main(int argc, char* argv[])
{
  ...
  printf("[EE405 Datagram Server Example]\n"); 
  ...
  /* 3. recvfrom() & sendto(): echo server */
  while(true) {
    printf("server: waiting request.\n");
    nbytes = recvfrom(server_socket, message, 255, 0, (struct sockaddr*)&client_addr, &client_addr_len);
    if (nbytes < 0) {
    ...
    nbytes = sendto(server_socket, message, strlen(message), 0, (struct sockaddr*)&client_addr, client_addr_len);
